...
def plot_img(img, range=[0, 255]):
    matplotlib.pyplot.imshow(img, vmin=range[0], vmax=range[1])
    matplotlib.pyplot.xlabel("xpixels")
    matplotlib.pyplot.ylabel("ypixels")
...
def plot_query_retrieval(img_query, imgs_retrieval, outFile):
    ...
    fig = matplotlib.pyplot.figure(figsize=(2*n_retrieval, 4))
    fig.suptitle("Image Retrieval (k={})".format(n_retrieval), fontsize=25)

    # Plot query image
    ax = matplotlib.pyplot.subplot(2, n_retrieval, 0 + 1)
    matplotlib.pyplot.imshow(img_query)
    ...
    for i, img in enumerate(imgs_retrieval):
        ax = matplotlib.pyplot.subplot(2, n_retrieval, n_retrieval + i + 1)
...
def plot_tsne(X, imgs, outFile):
    ...
    def plot_embedding(X, imgs, title=None):
        ...
        for i in range(X.shape[0]):
            matplotlib.pyplot.text(X[i, 0], X[i, 1], ".", fontdict={'weight': 'bold', 'size': 9})
...
def plot_reconstructions(imgs, imgs_reconstruct, outFile,
    ...
    fig = matplotlib.pyplot.figure(figsize=(20, 4))
    fig.suptitle("Image Reconstructions", fontsize=35)
    n = min(len(imgs), 10)
    for i in range(n):

        # Plot original image
        ax = matplotlib.pyplot.subplot(2, n, i + 1)
        matplotlib.pyplot.imshow(imgs[i],
                   vmin=range_imgs[0],
                   vmax=range_imgs[1])
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
        ...
        ax = matplotlib.pyplot.subplot(2, n, n + i + 1)
        matplotlib.pyplot.imshow(imgs_reconstruct[i],
